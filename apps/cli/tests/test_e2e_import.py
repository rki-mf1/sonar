...
def test_wfa_anno_no_upload_covid(monkeypatch, api_url, tmpfile_name):
    ...
    command = f"import --db {api_url} -r MN908947.3 --method 3 --fasta ../../../test-data/sars-cov-2/seqs.fasta.gz --cache {tmpfile_name}/wfa -t 2 --no-upload --auto-anno --must-pass-paranoid --skip-nx"
    code = run_cli(command)
