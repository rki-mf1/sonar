...
def test_parasail_no_anno_no_upload(monkeypatch, api_url, tmpfile_name):
    ...
    command = f"import --db {api_url} -r MN908947.3 --method 2 --fasta ../../../test-data/sars-cov-2/seqs.fasta.gz --cache {tmpfile_name}/parasail -t 2 --no-upload"
...
def test_add_sequence_mafft_anno_prop(monkeypatch, api_url, tmpfile_name):
    ...
    #     ),
    # )
    command = f"import --db {api_url} -r MN908947.3 --method 1 --fasta ../../../test-data/sars-cov-2/seqs.fasta.gz --cache {tmpfile_name}/mafft -t 2 --auto-anno --tsv sars-cov-2/meta.tsv --cols name=IMS_ID collection_date=DATE_DRAW sequencing_tech=SEQ_REASON sample_type=SAMPLE_TYPE "
    code = run_cli(command)
...
def test_add_sequence_mafft_no_skip(monkeypatch, api_url, tmpfile_name):
    ...
    code = run_cli(
        f"import --db {api_url} -r MN908947.3 --method 1 --fasta ../../../test-data/sars-cov-2/seqs.fasta.gz  --cache {tmpfile_name}/mafft  --no-skip --no-upload"
    )
    assert code == 0
...
def test_add_sequence_mafft_skip(monkeypatch, api_url, tmpfile_name):
    ...
    code = run_cli(
        f"import --db {api_url} -r MN908947.3 --method 1 --fasta ../../../test-data/sars-cov-2/seqs.fasta.gz --cache {tmpfile_name}/mafft -t 1 --no-upload"
    )
    assert code == 0
