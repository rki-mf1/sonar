<template>
  <div :class="filterGroup.marked ? 'filter-group marked' : 'filter-group'">
    <div v-for="filter in filterGroup.filters?.propertyFilters" class="single-filter">
      ...
      <Calendar
        v-if="filter.propertyName?.includes('date')"
        v-model="filter.value"
        style="flex: auto"
        dateFormat="yy-mm-dd"
      />
      <Dropdown
        v-else-if="fetchOptionsProperties.includes(filter.propertyName)"
    ...
    <div class="button-bar">
      ...
      <Button
        ...
        icon="pi pi-filter"
        :disabled="
          filterGroup.filters.propertyFilters.length + filterGroup.filters.profileFilters.length ==
          0
        "
      />
    </div>
