...
def delete_sample(sample_list: list):
    ...
    data["deleted_samples_count"] = deleted_sample[1]['rest_api.Sample'] if deleted_sample[0] != 0 else 0
...
def delete_sample_old(reference_accession, sample_list: list):
    ...
    """
    data = {"NOTE": "These are the numbers that need to be deleted.", 
            "samples_count": 0, 
            "properties_count": 0,
            "alignments_count": 0,
            "seqhashes_count": 0
    ...
    filtered_ref_sample = Sample.objects.filter(sequence__alignments__replicon__reference__accession=reference_accession, name__in=sample_list)
    if DEBUG:
        print("------------")
